...
void CWaveView::OnDraw(CDC* pDC)
{
	...
	for(int i=2;i<cp.x*20;i++)
	{
		...
		pDC->LineTo(i,((buffer[i-2+105]+buffer[i-1+105]+buffer[i+105]+buffer[i+1+105]+buffer[i+2+105])/5-128)*cp.y/170);
		//pDC->MoveTo(i,0);
  ...
  HPEN hPen=CreatePen(PS_DOT,1,RGB(0,0,255)); //установка красного пера
		pDC->SelectObject(hPen);
pDC->MoveTo(2,0);
	for(int j=2;j<cp.x*20;j++)
	{
	...
	for(int k=4;k<cp.x*20;k++)
	{
		...
		//pDC->LineTo(i,((buffer[i-2+105]+buffer[i-1+105]+buffer[i+105]+buffer[i+1+105]+buffer[i+2+105])/5-128)*cp.y/200);
		//pDC->MoveTo(i,0);
		pDC->LineTo(k,((buffer[k-4+105]+buffer[k-3+105]+buffer[k-2+105]+buffer[k-1+1+105]+buffer[k+105]+buffer[k+1+105]+buffer[k+2+105]+buffer[k+3+105]+buffer[k+4+105])/9-128)*cp.y/180);
